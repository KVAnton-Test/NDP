...
<style type="text/css">
  ...
  class PhotoTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @filename
        ...
        "<a href=\"#{p.path_for(@filename)}\" class=\"#{classes.join(" ")}\" data-fancybox data-caption=\"#{@title}\"><img src=\"#{p.thumb_for(@filename,@thumb)}\" #{dimensions} alt=\"#{@title}\" /></a>"
      else
  ...
  class GalleryTag < Liquid::Block
    ...
    def render(context)
      ...
      md5 = Digest::MD5.hexdigest(super)
      ...
      "<div class=\"f-carousel adaptive-height \"> #{list} </div>"
    end
  end
